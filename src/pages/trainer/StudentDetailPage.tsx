...
const PLAN_LABELS: Record<string, string> = {
  ...
  cambios_personales: "Cambios Personales",
};

const PLAN_ICONS: Record<string, React.ElementType> = {
  nutricion: Apple,
  entrenamiento: Dumbbell,
  cambios_fisicos: TrendingUp,
  cambios_personales: User,
};
...
export default function StudentDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [profRes, exRes, plRes] = await Promise.all([
      supabase.from("profiles").select("display_name, avatar_initials, weight, age").eq("user_id", studentId).single(),
  ...
  if (!profile) {
    return (
      <div className="space-y-4">
        <Button variant="ghost" onClick={() => navigate("/trainer/students")} className="gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <Card className="card-glass">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Niveles</p>
          </CardContent>
        </Card>
        <Card className="card-glass">
          <CardContent className="p-4 text-center">
